import type { ActivityItem } from "../server/mock-data";

type AgentActivityPanelProps = {
  items: ActivityItem[];
};

export function AgentActivityPanel({ items }: AgentActivityPanelProps) {
  return (
    <aside className="panel panel--activity">
      ...
      <div className="activity-list">
        {items.map((item) => (
          <article className="activity-item" key={`${item.title}-${item.timestamp}`}>

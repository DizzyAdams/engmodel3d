...
export class Orchestrator {
    ...
    compliance;
    versions;
    constructor(projects = new ProjectStore(), queue = new TaskQueue(), validator = new ValidationEngine(), compliance = new ComplianceEngine(), versions = new VersionStore()) {
    ...
    createPlan(request) {
        ...
        const cadProfile = project.kind === "mechanical"
            ? "bracket"
        ...
        const tasks = steps.map((step) => this.queue.enqueue({
            role: step.role,
            title: step.title,
            input: request.brief,
            ...
            rules: [
                {
                    ...
                    validate: (subject) => subject.project.name.trim().length > 0
                        ? []
                        : [
                            {
            ...
            rules: [
                ...
                },
                {
                    ...
                    validate: (value) => typeof value === "object" &&
                        value !== null &&
                        "metadata" in value &&
                        typeof value.metadata?.projectId === "string",
                },
            ],
        }, {
            project,
